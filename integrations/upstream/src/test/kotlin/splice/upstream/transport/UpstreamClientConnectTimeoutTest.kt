// G11: TCP connect used to ride the 5-minute first-byte knob (connectTimeoutMillis was wired to
// the same knob as the headers/body-wait phase), so a blackholed/dead address stalled to the OS
// SYN timeout x maxRetries instead of failing fast into the transport-retry loop. This pins
// defaultClient()'s connectTimeoutMillis at a fixed 10s, decoupled from the caller's total cap,
// the only timeout a caller sets since V4-125 (it bounds the call and each read, never connect).
...
class UpstreamClientConnectTimeoutTest {
    ...
    @Test
    fun `connect timeout fires around 10s, decoupled from a much larger total cap`() {
        val client = UpstreamTransport().defaultClient(totalTimeoutMs = 900_000L)
        ...
        assertTrue(
            elapsedMs < 20_000,
            "connect must fail fast (~10s), not ride the 900_000ms total cap; took ${elapsedMs}ms",
        )
        assertTrue(
