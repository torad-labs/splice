...
name: bug-catcher
description: >-
  Build immunity against recurring bug classes. Each caught pattern strengthens
  immune memory for that class (null-deref, race-condition, injection, etc).
  Prescribes structural fixes that prevent entire bug CLASSES from recurring,
  not just patches for individual instances. Triggers: "we've seen this bug
  before", "this keeps happening", "recurring failures", "post-mortem",
  "what's our most common bug type", or "/bug-catcher".
user-invocable: true
context:
