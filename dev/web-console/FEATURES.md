...
| `GET /api/compaction/instructions?head=<key>` (decided 2026-09-18 with the daemon lead, V4-136: serves 4.10 'Effective instructions'; the compaction page claims 4.10 once the row lands) | under the bearer; `{ scopes: [{ scope: 'global' \| 'project' \| 'session', source: <path or 'none'>, chars: number }] }` computed by the same resolver core applies to a compaction; text by length only, no instruction text over the wire; unknown head answers 400 naming it | the compaction instruction resolver |
