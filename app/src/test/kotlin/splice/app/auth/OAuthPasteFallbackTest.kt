...
class OAuthPasteFallbackTest {
    ...
    fun `a pasted redirect URL yields its code`() {
        assertEquals(
            "abc123XYZ",
            flow.extractCode("http://127.0.0.1:1455/auth/callback?code=abc123XYZ&state=s1"),
        )
        assertEquals(
            "abc123XYZ",
            flow.extractCode("  http://localhost:8080/callback?state=s1&code=abc123XYZ  "),
    ...
    fun `a fragment-delivered code is found too`() {
        assertEquals("frag-code-1", flow.extractCode("http://127.0.0.1:1455/cb#code=frag-code-1"))
    }

    @Test
    fun `a pasted URL code is decoded once before form encoding`() {
        val encoded = "code%2Fpart%3Dvalue%2525"
        val code = flow.extractCode("http://127.0.0.1/callback?code=$encoded&state=s1")
    ...
    fun `a bare code is accepted`() {
        assertEquals("ac_01HXYZabcdef", flow.extractCode("ac_01HXYZabcdef"))
    }
    ...
    fun `noise is rejected rather than exchanged`() {
        assertNull(flow.extractCode(""), "a bare Enter is not a code")
        assertNull(flow.extractCode("   "), "whitespace is not a code")
        assertNull(flow.extractCode("y"), "a stray keystroke is not a code")
        assertNull(flow.extractCode("no thanks"), "prose is not a code")
        assertNull(
            flow.extractCode("https://accounts.x.ai/sign-in"),
