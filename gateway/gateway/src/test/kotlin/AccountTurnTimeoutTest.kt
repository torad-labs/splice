...
class AccountTurnTimeoutTest {
    ...
    fun `expired continuation uses the canonical watchdog terminal without an upstream call`() = runBlocking {
        ...
        try {
            ...
            val actualTag = drive.pipeline.finishStream(rig.terminal, actual, drive.meta, 30_000L, false)
            val expectedTag = drive.pipeline.finishStream(expectedTerminal, expected, drive.meta, 30_000L, false)
