...
| Transcript | Read the session's conversation: user turns, assistant output, tool calls and results, system prompt, paginated and searchable. Source: the client's own local transcript file, joined by `session_id`, under the head's config dir (`CLAUDE_CONFIG_DIR/projects/<slug>/<session_id>.jsonl`), which is the shared vanilla `~/.claude/projects` tree for a head whose policy shares `projects` (the default and the operator's own policy: any head resumes any session, V4-64 re-landed by V4-168 on 2026-09-19 after V4-115 had made the trees private) and a real private directory for a head that isolates it (`ProjectsLink.kt`); the reader falls back per id across the vanilla tree and every head tree and shows which path it read. Read as a conversation between participants, not a log tail. For clients with no local transcript, the opt-in body capture in §4.9. Never sent anywhere; read from disk on demand. | route | operator 2026-09-17 |
...
| Claude logins | Several Claude logins on the splice-owned Claude head (`claude-splice`, its own config dir), saved and switched only by `splice login <claude-head> --label <name>` while no session of the head runs, the live login saved under its own account's label first (V4-276); a launch never writes the head's credential file, because Claude Code's refresh tokens are single-use and a restored superseded one gets the login revoked (V4-237, V4-250). Traffic still flows through Claude Code's own login, splice never calls Anthropic with a consumer token. This is not the account pool: `auth = { kind = "client" }` builds no pool, no poller, no availability, no exclusion and no per-turn selection (`HeadAccountPools.kt:27,54`), and the credential file is per head, so every live session on the head shares one login and a swap while a session runs collides with that session's own token refresh. The console renders Claude's selected login, switched only by that verb, no mid-session switch, no next target; the constraint (one login per Claude head at a time, or a config dir per session, which is a later design) is printed on the strip. Never on a wrapped default head. Ruled in by the operator on 2026-09-17; Anthropic's consumer terms reserve OAuth for Claude Code and claude.ai, and the operator owns that reading. | new | operator 2026-09-17 |
...
| `GET /api/models` | catalog per head. A row is section 4.8's columns: id, label, description, `slot`, context window and `context_window_source`, rates, pinned, plus `provider: string`, the registry's provider key for the head (decided 2026-09-18 with the daemon lead, V4-127: the models page groups by provider). `slot` is the DECLARED tier from `HeadConfig.models`, joined onto the resolved catalog, and the join is reported in both directions because 4.8's column is literally "the tiers Claude Code will and will not get on this head": a resolved model that was never declared carries `slot: null`, and a declared slot that resolved to nothing is its OWN row with `resolved: false` and a reason, never dropped. A console that silently omits the tier the operator is missing is the one thing this page must not do. `context_window_source` is the provenance of THAT ROW'S context window value (spelled that way, and NOT `windowSource` or `window_source`, ruled 2026-09-18 after this line was found carrying a third spelling of a field the daemon already emits: it pairs with `context_window` on the same row so the JSON needs no inference, and a name that needs the next sentence of its own contract to say WHICH window is the wrong name. A field we author on a surface we own is snake_case with the unit or the subject in the name; a field a foreign protocol names keeps that protocol's spelling, which is why `protocolVersion` and `hookEventName` are not precedents here), not the origin of the model entry — 4.8 asks for "context window and its source". It is a label the daemon composes and the console never parses. The origin of the entry needs no field today: clarified 2026-09-18 for V4-127, the catalog is declared models only (see the end of this section), so every row is declared; add an origin field in the same change that adds the remote provider catalog listing, never before. | `ModelCatalog`, `Topology` |
...
| `GET /api/heads/{head}/trace?last=&session=&turn=` (V4-239) | what `splice trace` prints: the newest turns on the head's trace day files, oldest first, with no body; `?turn=ID` is that turn's records as written (headers redacted when written, bodies exact up to the cap). Served whether capture is on now or not, since the files outlive the knob; the drawer labels files kept from a capture since turned off. `--purge` stays on the CLI | `TraceRows`, the verb's own reader |
| `GET /api/heads/{head}/wire?last=` (V4-239) | what `splice wire` prints: the head's wire tap, its last kept upstream request bodies; a head with no tap answers 409 with the head's own "tap is off" sentence, never an empty list | `WireTap`, registered per head by `HeadServerFactory` |
| `GET /api/models/upstream?provider=` (V4-239) | what `splice models` prints: each provider's published list against splice.toml, every declared row with its verdict and every served model no row declares, or why a provider publishes none or could not be read. Asked of the providers on every call, so the console reads it when Compare is pressed and never polls; no credential in the answer (a `models_url`'s user info, query and fragment are cut); an unknown provider answers 400 naming the declared ones | `ModelsReporter`, the verb's own comparison |
