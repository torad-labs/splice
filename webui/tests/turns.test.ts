...
describe('turn strips', () => {
  ...
  test('a column the row does not carry is absent and says so', () => {
...
describe('turns board', () => {
  ...
  test('the summary bay renders what exists today, and an empty window says so', () => {
    ...
    expect(out).toContain('no turns in window');
    expect(out).toContain('>unavailable<'); // no percentile was computed for an empty window
  });
...
describe('logs board', () => {
  const board = (over: Partial<React.ComponentProps<typeof LogsBoard>> = {}) =>
    render(
      h(LogsBoard, {
        payload: { key: 'claudex', path: '/home/marcos/.claude-codex/logs/daemon.log', lines: [] },
        filter: NO_FILTER,
        follow: true,
        appended: 0,
        reset: false,
  ...
  test('an empty tail names the path it read', () => {
    ...
    expect(out).toContain('/home/marcos/.claude-codex/logs/daemon.log');
  });
