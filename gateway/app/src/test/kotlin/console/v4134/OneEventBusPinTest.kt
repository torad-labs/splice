...
class OneEventBusPinTest {
    ...
    fun `the daemon hands every head the control plane's publisher`() {
        assertTrue(
            source("gateway/app/src/main/kotlin/splice/app/Daemon.kt").contains("console = controlPlane.console"),
    ...
    fun `the head factory gives each head its own keyed reporter`() {
        assertTrue(
            source("gateway/app/src/main/kotlin/splice/app/head/HeadServerFactory.kt")
