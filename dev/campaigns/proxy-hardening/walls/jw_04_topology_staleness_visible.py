...
def derived_mutants() -> list[str]:
    ...
    live = [_read(CONTROL), _read_shell(SHIM), _read(DOCTOR)]
    if detect(*live):
        return ["derived mutants need the live tree green; the wall is RED right now"]
    fails = []
    for where, tokens in ((0, ["topologyDigest"]), (0, ["topologyStale"]), (1, ["topologyStale", "topologyDigest"]), (1, ["splice restart"]), (2, ["topologyDigest", "topologyStale"])):
