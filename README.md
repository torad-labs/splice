...
| File | What it holds | Who can read it | How long it stays | Written by |
| --- | --- | --- | --- | --- |
| `~/.config/splice/splice.toml` | Your topology: providers, heads, models and ports, and any header values you put in `extra_headers` | Only you (0600), each backup too: the daemon holds both owner-only at every start, including ones an older splice left open | Yours. Written once from a template on first run. A console edit first saves the old bytes as `splice.toml.bak-<time>-<hash>` beside it, one per distinct version, and keeps the ten newest; `splice add` edits it with no backup | `TopologyLoader.kt`, `TopologyWriter.kt`, `AddWrite.kt` |
