...
// the 14-function ceiling — hence a collaborator of its own.
 ...
 *  the pipeline. [runningJar] is the executable's answer to where it runs from. Every member keeps
 *  the old function's name. */
internal class DoctorInstallProbes(private val probes: DoctorProbes, private val runningJar: RunningJar) {
    ...
    private fun jarCheck(): DoctorCheck {
        ...
        val failure = Cancellables.runCatchingCancellable { Files.getLastModifiedTime(jar) }.exceptionOrNull()
