...
// file. Twenty members moved out into three packages — splice.head.round (the round runners,
// which already had zero dependence on TurnDrive/TurnDriver), sibling files in
// splice.head (failure classification, honest-ending, per-turn context assembly), and
// splice.head.wire (the client write surface, which that package already owns the rest of).
...
// splice.head.round; ClientChannel.kt, TurnWiring.kt in splice.head.wire.
...
import splice.head.HeadHealthCounts
import splice.head.admission.LocalRefusal
import splice.head.admission.LocalResponses
