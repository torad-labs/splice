...
/* SECTION HEADER (h2 + tooltip inline) */
.section-header {
    display: flex;
...
/* TIP — inline info icon with hover tooltip */
.tip {
    position: relative;
    display: inline-flex;
    align-items: center;
...
/* Clamp tooltip to viewport on narrow containers */
@media (max-width: 640px) {
    .tip::after { width: 200px; font-size: 11px; }
}
...
/* enabled/disabled variants — for generate mode mechanism configuration */
.component-card.enabled { border-left: 3px solid var(--accent); }
.component-card.disabled { border-left: 3px solid var(--red-soft); }
.component-tag {
    font-size: 9px;
    font-weight: 700;
...
.component-tag.enabled { color: var(--accent); }
.component-tag.disabled { color: var(--red-soft); }
...
/* .metric — coupling/score value shown below description */
...
#### `.perspective-card` / `.perspective-map`

Shows a single perspective examined during analysis — survived or cut. Shows the pressure score (how hard the field pushed against it) and the reason for its fate. Use in rejection.html to show cut perspectives ("What Was Considered and Cut") and in prd.html to show surviving perspectives ("What Made It Through").
...
.perspective-card.survived { border-left: 3px solid var(--accent); }
.perspective-card.cut { border-left: 3px solid var(--red-soft); }
.perspective-card h4 { font-size: 15px; font-weight: 600; margin-bottom: 6px; }
.perspective-card p { font-size: 13px; color: var(--text-muted); margin: 0; line-height: 1.5; }
.pressure-score {
    font-size: 11px;
    font-weight: 700;
    padding: 3px 10px;
...
/* Higher pressure = bolder color — strong pressure on a survived perspective = important finding */
.pressure-score.high { background: var(--amber-dim); color: var(--amber); }
.pressure-score.very-high { background: #fdf6f6; color: var(--red-soft); }
```

Score thresholds for pressure: `.high` = 0.60+, `.very-high` = 0.80+. Below 0.60 use no modifier.

```html
<!-- Survived perspective -->
<div class="perspective-card survived">
    <div>
        <h4>Perspective name</h4>
        <p>Why it survived and what it contributes.</p>
    </div>
    <span class="pressure-score high">δ 0.73</span>
</div>

<!-- Cut perspective -->
<div class="perspective-card cut">
    <div>
        <h4>Perspective name</h4>
        <p>Why it was cut — the structural reason, not the opinion.</p>
    </div>
    <span class="pressure-score">δ 0.42</span>
</div>
```
...
@media (max-width: 640px) {
    ...
    .pressure-score { justify-self: start; margin-top: 6px; }
}
```
...
Shows the type of a learned pattern (in the "Patterns Catalogued" section). Three variants map to how the pattern was discovered.
...
.pattern-type-pill.assumption-challenged { background: var(--amber-dim); color: var(--amber); }
.pattern-type-pill.design-choice { background: var(--survived); color: var(--accent); }
.pattern-type-pill.bias-caught { background: var(--indigo-dim); color: var(--indigo); }
```
...
<ul class="patterns-list">
    <li>
        <span class="shield">&#9681;</span>
        <span>
            <span class="pattern-type-pill assumption-challenged">Assumption Challenged</span>
            <strong>Pattern name.</strong> What the default assumption was, and what evidence showed instead.
        </span>
    </li>
    <li>
        <span class="shield">&#9681;</span>
        <span>
            <span class="pattern-type-pill design-choice">Design Choice</span>
            <strong>Pattern name.</strong> A deliberate deviation from the default, backed by evidence.
        </span>
    </li>
    <li>
        <span class="shield">&#9681;</span>
        <span>
            <span class="pattern-type-pill bias-caught">Bias Caught</span>
            <strong>Pattern name.</strong> A training distortion that was identified and overridden.
        </span>
    </li>
</ul>
```
...
| Regression risk | Regression probability | P(breaking existing behavior) |

### Perspective / vertex translations

| Engine Term | Reader Label | Notes |
|---|---|---|
| Vertex | Perspective | A viewpoint or angle examined during analysis |
| Survived (vertex) | Survived | Held up under pressure — contributes to the plan |
| Pruned (vertex) | Cut | Didn't survive — documented so it's clear it was considered |
| Delta (vertex) | Pressure score (δ) | How hard the analysis pushed against this perspective. Higher = more pressure, more significant. Show as `δ 0.73`. |
| Navigate phase | Perspective discovery | The initial phase that finds and pressure-tests viewpoints |
| Convergence | Refinement | The iterative tightening loop that improves quality |
| Navigate output | Perspectives explored | The full set of viewpoints, survived and cut |

### Pattern / antibody translations

| Engine Term | Reader Label | Notes |
|---|---|---|
| Antibody (individual item) | Pattern | A learned behavior to repeat or avoid |
| `training_pull` | Assumption Challenged | The default assumption was overridden by evidence |
| `default_override` | Design Choice | Deliberate deviation from the default, evidence-backed |
| `bias_caught` | Bias Caught | Training distortion identified and named |
| Antibody strength | Pattern confidence | 0–1 signal strength. 0.70+ = strong. Show if notable. |
| Immune Memory (section heading) | Patterns Catalogued | |
| Antibody Deposit (section heading) | Patterns Catalogued | |
| Training pull (in assumption cards) | Default assumption | |
...
### Score explanations (for tooltips and inline notes)

Use these as the canonical explanation text when adding `.tip` tooltips to score-bearing sections:

| Section / Score | Tooltip text |
|---|---|
| Overall quality score (ELBO) | "How well this analysis captured the idea. Accuracy = how closely the output matches the input. Complexity = how efficiently it gets there. 0.70+ is strong." |
| Pressure score (δ) | "How hard the analysis pushed against this perspective. Higher pressure on a survived perspective = important, non-obvious finding." |
| Quality bar: Coherence | "Theme consistency across all perspectives. High = the analysis stayed focused and didn't wander." |
| Quality bar: Spread | "How much architectural surface the analysis covered. High = broad coverage. Low = narrow focus." |
| Quality bar: Complexity flow | "Whether complexity is distributed well — builds toward the core, doesn't spike randomly." |
| Quality bar: Exploration | "How much solution space was covered before converging. Low = may have missed alternatives." |
| Quality bar: Density | "Information per unit of structure. High = rich, compressed findings. Low = thin." |
| Quality bar: Readiness | "How prepared this plan is for implementation. Low = more decisions needed before building." |
| Quality bar: Convergence | "Whether each iteration improved quality. Low = the loop stalled." |
| Quality bar: Signal | "Felt quality of the process — hard to fake, reliable indicator." |
| Quality bar: Decisiveness | "Whether deliberation added value. High = good use of iteration. Low = spinning." |
| Quality bar: Contamination | "How much default/training bias remains. Low is better — lower contamination = more evidence-driven." |
| Quality bar: Bias | "Residual training distortion after analysis. Low is better." |
| Quality bar: Pattern memory | "Strength of immune memory deposited. High = strong patterns catalogued for future use." |
| Quality bar: Integration | "How well this plan connects to existing architecture. Low = isolated, may create friction." |
| Quality bar: Constraints | "How tight the implementation path is. High = clear path. Low = many open decisions." |
| Quality bar: Feasibility | "Gap between what's desired and what's possible. Low gap = achievable. High gap = risk." |
| Quality bar: Regression probability | "Risk that this plan breaks existing behavior. Low is better." |
...
**Goal:** Scannable in 30 seconds. Hub with navigation to companion documents. Assume zero preparation budget — the reader already knows why they're here, deliver the conclusion then point to evidence.
...
**Standard tooltips for index.html:**
- Executive Summary h2: `"The 60-second version. What the analysis found and what it means for the build. Full analysis in the companion documents."`
- Overall quality score h2: `"How well this analysis performed. Accuracy = how closely the output captures the idea. 0.70+ is strong."`
- Patterns Catalogued h2: `"Behaviors to repeat or avoid in future work on this topic. Each pattern is a decision shortcut."`
...
Include a "What Was Considered and Cut" section using `.perspective-map` with `.perspective-card.cut` cards to show which perspectives didn't survive — documenting the reasoning so the reader knows these paths were explored, not overlooked.

Include only the sections that have substantial content for the current mode.

**Standard tooltips for rejection.html:**
- What Was Considered and Cut h2: `"Perspectives that didn't hold up under pressure. Documented here so you know they were explored — these are ruled out, not missed."`
- Pressure score label: `"How hard the analysis pushed against this perspective. Higher pressure on a cut perspective = it was a seductive trap — felt right, but failed under scrutiny."`
- (generate) Assumptions Overturned h2: `"What we thought going in vs what the evidence showed. These are the places where the default was wrong."`
- (verify) What to Stop Doing h2: `"Patterns in the current code that are diverging from requirements. Stop these before they compound."`
- (diagnose) What Failed h2: `"The structural description of failure — not the symptom, the architecture that made failure inevitable."`
- Patterns Catalogued h2: `"Anti-patterns to avoid. Each one is a behavior that looked right but caused problems."`
- When to Revisit h2: `"Conditions under which these cut decisions should be re-evaluated. Ruled out under current conditions — not permanently."`
...
Include a "What Made It Through" section using `.perspective-map` with `.perspective-card.survived` cards to show the perspectives that survived — each with its pressure score and what it contributes to the plan. This is the viewpoint portfolio the plan is built on.

Include only the sections that have substantial content for the current mode.

**Standard tooltips for prd.html:**
- What Made It Through h2: `"Perspectives that held up under pressure. The plan is built from these viewpoints. Higher pressure score = the finding is more non-obvious and valuable."`
- Pressure score label: `"How hard the analysis pushed against this perspective. High pressure + survived = important, non-obvious insight worth paying attention to."`
- (generate) Architecture h2: `"Concrete plan: files to create or modify, how data flows, what to build in what order."`
- (generate) Open Questions h2: `"Points that need more evidence before building. Don't skip these — they're where risk lives."`
- Quality Assessment h2: `"16-dimension measurement of this plan. Strong (teal) = confident. Adequate (amber) = some uncertainty. Weak (red) = risk zone that needs attention."`
- Overall quality score h2: `"How well this plan crystallized. Accuracy = how closely it captures the idea. Complexity = efficiency. 0.70+ is strong. Below 0.60 = revisit."`
- Patterns Catalogued h2: `"What we learned from this analysis. Each pattern is a decision shortcut for the next time you encounter this type of problem."`
- (verify) What's Working h2: `"Areas where the code correctly matches requirements. Keep these intact during fixes."`
- (compare) Why This One Wins h2: `"The dimensions that decided the comparison. Check these if you want to challenge the recommendation."`
- Out of Scope h2: `"What this configuration deliberately does not cover. Explicit boundaries prevent scope creep."`
- Success Criteria h2: `"How you know this voice state is working. Concrete, testable."`
...
2. Every section uses `.section-label` + `.section-header` (wrapping h2 + `.tip`) pattern. The `.tip` tooltip text comes from the Section F standard tooltip table for known section types, or is written inline for custom sections.
3. Quality measurement appears in every document (full 16-dimension in single-doc and PRD; snapshot in product index). Each `.quality-label` cell includes a `.tip` inline with the tooltip text from the Section E score explanations table.
4. Patterns catalogued appears in every document. Include `.pattern-type-pill` for each pattern item.
...
11. All reader-facing labels must use the translation tables in Section E. Never expose engine terms (vertex, antibody, navigate, convergence, ELBO, delta) directly — always use the reader-facing equivalent.
12. Product PRD includes a "What Made It Through" section with `.perspective-card.survived` cards. Product rejection includes a "What Was Considered and Cut" section with `.perspective-card.cut` cards. PRD also includes "Out of Scope" (explicit non-goals, using `.findings-grid` with red-soft left border) and "Success Criteria" (testable conditions, using `.flow-steps`). Rejection also includes "When to Revisit" (ADR-style revisit conditions using `.questions-list`).
13. **Product output sentence filter.** Every sentence in a product document (index.html, rejection.html, prd.html) must either reduce time-to-decision or be cut. No orientation paragraphs ("In this document you will find…"), no appreciation framing ("This analysis revealed a fascinating…"), no reading journey invitations ("Start with either, but read both"). State the conclusion, then provide the evidence. The reader's energy budget is zero.

**Section header pattern (required for all sections):**
```html
<section>
    <div class="section-label">LABEL</div>
    <div class="section-header">
        <h2>Section Title</h2>
        <span class="tip" data-tip="Tooltip text from Section F or written inline.">i</span>
    </div>
    <!-- section content -->
</section>
```

**Quality label with tooltip (required for quality grid):**
```html
<div class="quality-label">Coherence <span class="tip" data-tip="Theme consistency across all perspectives. High = the analysis stayed focused and didn't wander.">i</span></div>
```
