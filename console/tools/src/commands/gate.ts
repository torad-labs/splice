...
import { THEMES, themeSeedSource } from '../lib/theme.ts';
...
// `gate sheets <milestone>`: the contact-sheet captures, folded in from gate.mjs by the restructure.
if (ARGS[0] === 'sheets') {
  await milestoneSheets(ARGS.slice(1));
  process.exit(process.exitCode ?? 0);
}

// The repo root, derived from THIS file's home rather than from the caller's cwd (M1-16: the gate
// moved here from console/.impeccable/review/gate/, which is gitignored). Every other default below
// is already relative to the root, so a row can run `bun console/tools gate` from the
// worktree root or from anywhere else and read the same tree.
const HERE     = path.dirname(fileURLToPath(import.meta.url));
const ROOT     = flag('root', path.resolve(HERE, '..', '..', '..', '..'));
const CAPTURES = flag('captures', 'console/.impeccable/review/sections');
...
const AXES_DIR = flag('axes', 'console/.impeccable/review/axes');
const SRC      = flag('src', 'console/src');
const TOKENS   = flag('tokens', 'console/src/shared/tokens.css');
const COMP     = flag('comp', 'console/tools/fixtures/comp/team-board-a.png');
 ...
 * (console/src/widgets/log-tail/log-tail.css), the SAME INK as a cell divider. So the leg paired the
...
function checkFieldGrid(capturesDir) {
  ...
  // console/src/app/rows.ts — the same list the capture leg enumerates and the same one the tonal
  ...
  const detail = (bad.length ? bad.join(' · ') : `aligned on ${checked} captures`)
    ...
    + (staleCount ? ` · ${staleCount} skipped (older than console/src, so not evidence about this build)` : '');
...
function checkTonalDrift(capturesDir, compPath) {
  ...
  // console/src/app/rows.ts -- the same list the capture leg enumerates: one fresh capture per
...
function seedCaptures(tmp, addresses, { im, strips }) {
  ...
  fs.mkdirSync(path.join(tmp, 'console/src/app'), { recursive: true });
  fs.writeFileSync(path.join(tmp, 'console/src/app/rows.ts'),
...
/** THE GATE PRODUCES WHAT IT JUDGES. It read `console/.impeccable/review/sections`, a directory it
...
const ADDRESSES_FILE = 'console/src/app/rows.ts';
 ...
 * the ADDRESSES table in console/src/app/rows.ts, the same list the capture leg enumerates; the legs
 ...
 *  to measure is the console's own source, so the newest mtime under console/src is the bar. */
 ...
 * another seat wrote `console/src/pages/settings/settings.css` at 10:57:07 during a 79-second capture
...
function selftest() {
  ...
  const cases = [
    ...
    ['page-coverage', () => {
      ...
      // THE FRESHNESS BAR IS `console/src`, AND THIS FIXTURE WRITES rows.ts LAST, so without this the
    ...
    ['field-grid', () => {
      ...
      at('console/src/app/rows.ts', t);                      // the source, as the run begins
      ...
      fs.writeFileSync(path.join(tmp, 'console/src/app/later.css'), '.x { color: red; }');
      at('console/src/app/later.css', t + 20_000);
      checkFieldGrid('caps'); ROOTREF.root = saved;
      fs.rmSync(tmp, { recursive: true, force: true });
...
// sample, so a write into console/src while the browser is working can no longer retroactively
...
if (freshness && freshness.stale > 0) {
  process.stderr.write(`  ! captures: ${freshness.stale} of ${freshness.files.length} are older than console/src (oldest ${freshness.oldestName})\n`);
}
...
//
// Usage: bun console/tools gate sheets <milestone> [--dry-run]   (merged into this file by the
// restructure: `gate` is the look gate, `gate sheets` the contact sheets — one command, both named)
async function milestoneSheets(args) {
   ...
   * again (M1-28). One module now owns the mapping, and `bun console/tools fixtures`
  ...
  function rooms() {
    const sheet = readFileSync(join(ROOT, 'console/src/shared/tokens.css'), 'utf8');
    const found = {};
    let theme = 'dark';
  ...
  const [milestone, ...flags] = args;
  const dryRun = flags.includes('--dry-run');

  if (!milestone || !/^[a-z0-9-]+$/.test(milestone)) {
    console.error('usage: bun console/tools gate sheets <milestone> [--dry-run] [--out DIR]');
  ...
  const outDir = outArg === null ? join(ROOT, 'console/.impeccable/review/gate', milestone) : resolve(ROOT, outArg);
