...
| `GET /api/models` | catalog per head. A row is section 4.8's columns: id, label, description, `slot`, context window and `windowSource`, rates, pinned, plus `provider: string`, the registry's provider key for the head (decided 2026-09-18 with the daemon lead, V4-127: the models page groups by provider). `slot` is the DECLARED tier from `HeadConfig.models`, joined onto the resolved catalog, and the join is reported in both directions because 4.8's column is literally "the tiers Claude Code will and will not get on this head": a resolved model that was never declared carries `slot: null`, and a declared slot that resolved to nothing is its OWN row with `resolved: false` and a reason, never dropped. A console that silently omits the tier the operator is missing is the one thing this page must not do. `windowSource` is the provenance of THAT ROW'S context window value, not the origin of the model entry — 4.8 asks for "context window and its source". It is a label the daemon composes and the console never parses. The origin of the entry needs no field today: clarified 2026-09-18 for V4-127, the catalog is declared models only (see the end of this section), so every row is declared; add an origin field in the same change that adds the remote provider catalog listing, never before. | `ModelCatalog`, `Topology` |
