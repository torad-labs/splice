...
//
// `/api/models/upstream` is what `splice models [provider]` prints (V4-239): each provider's published
// roster against splice.toml, read when the operator asks, never polled, because every read reaches
// the providers.
...
export const dispositions: readonly Disposition[] = [
  ...
  // The Compare key: each provider's published roster against splice.toml, what `splice models` prints.
  { kind: 'verb', name: 'models', disposition: 'read-only', action: 'Compare the declared models with what each provider publishes', via: '/api/models/upstream' },
];
...
export const job: PageJob = {
  ...
  actions: [
    { name: 'Open a model' },
    { name: 'Add a model' },
    { name: 'Compare the declared models with what each provider publishes' },
  ],
};
