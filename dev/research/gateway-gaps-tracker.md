...
| L4 | FIR compiler plugin: `@MustConsume` discard = compile error (build AFTER L3 types exist). 2026-07-19: landed on L3 alone — L4 never needed G7 (the brief's precondition was stale; L3's `RefreshOutcome` is the only real dependency and it exists). New `:fir-checks` compiler-plugin module + `@MustConsume` in `:core`, wired tree-wide via `-Xplugin` in a new `gateway/build.gradle.kts`; proven red/green (K2JVMCompiler fixtures) plus a synthetic `:core` discard probe. `RefreshOutcome`/`doRefresh` deliberately NOT annotated here (separate, G7-gated scope) — the wall is proven against synthetic fixtures only. detekt overlay disables the ktlint rules that crash on context parameters at 1.23.8 (detekt#8140). | done 3285bf3 |
