...
class ModelsProbeTest {
    ...
    private val credentials = ModelCredentialSource { provider, key, readEnv ->
        credentialReads += key
        assertEquals("test", key)
        readEnv(provider.auth.effectiveApiKeyEnv(key))
    }
    ...
    fun `a responses provider and a client-auth provider are unpublished, not failures`() {
        ...
        assertTrue(asked.isEmpty())
        assertTrue(credentialReads.isEmpty(), "unpublished providers must not read credentials")
    }
    ...
    fun `a refusal says whether splice even held a credential`() {
        ...
        val none = ModelsProbe(http = { _, _ -> ModelsAnswer.Answered(401, "") }, credentials = credentials)
