...
- **Any head joins any session again (V4-168).** A session started on one head resumed on any
  ...
  session by name, as before. The one cost stays visible: a transcript carries the model id of the
  head that wrote it, so resuming it elsewhere prints Claude Code's "Session model X could not be
  restored" once and continues on the resuming head's default.
