...
//
// Usage: bun console/tools capture '<url>' <absolute out.png> [<width> <height>]
//        bun console/tools capture --sweep <dir> [<dir> ...]     check frames already on disk
//        bun console/tools capture --help
import { readdirSync, readFileSync, rmSync, statSync } from 'node:fs';
import { join, resolve } from 'node:path';
import { pathToFileURL } from 'node:url';
import { shoot, show, withChrome } from '../lib/cdp.ts';
import { THEMES, themeValues } from '../lib/theme.ts';
...
function paperColours() {
  const sheet = readFileSync(join(REPO, 'console/src/shared/tokens.css'), 'utf8');
  const found = [];
...
const HELP = `usage: bun console/tools capture '<url>' <absolute out.png> [<width> <height>] [--theme dark|light]
       bun console/tools capture --sweep <dir> [<dir> ...]
...
if (isMain && argv[0] === '--sweep') {
  ...
  if (dirs.length === 0) {
    console.error('usage: bun console/tools capture --sweep <dir> [<dir> ...]');
    process.exit(2);
  }
  const frames = [];
  const walk = (dir) => {
