...
//
// Usage: bun console/tools look '<url>' [--out DIR] [--width W] [--height H] [--json]
//
...
import { SELF_RUNTIME, run } from '../lib/proc.ts';
import { existsSync, readFileSync, statSync } from 'node:fs';
import { resolve } from 'node:path';
import process from 'node:process';
import { show, withChrome } from '../lib/cdp.ts';
import { decodePng } from '../lib/png.ts';
import { LOOK_DIR, nameFor, snapshot } from './snapshot.ts';
import { themeLanded, themeValues } from '../lib/theme.ts';
...
const GATE = resolve(import.meta.dirname, 'gate.ts');
...
function compReference() {
  const scaffold = 'console/.impeccable/build/scaffold/layout.css';
  const sizes = new Map();
  try {
...
if (has('ladder')) {
  ...
  if (target === undefined) {
    console.error('usage: bun console/tools look --ladder <url> [--width W] [--height H] [--theme dark|light] [--mutate]');
    process.exit(2);
  }
...
if (URL_ARG === undefined || has('help')) {
  console.log("usage: bun console/tools look '<url>' [--out DIR] [--width W] [--height H] [--theme dark|light] [--json]");
  console.log("       bun console/tools look --comp <png> [--json]   (the comp's type by ink area)");
...
let domError = null;
if (blocks.length === 0) {
  // the detector is the impeccable skill's own node script, run as it ships
  const detected = Bun.spawnSync(['node', DETECT, '--json', out], { stdout: 'pipe', stderr: 'pipe', stdin: 'ignore' });
  const stderr = detected.stderr.toString();
  try {
    domFindings = JSON.parse(detected.stdout.toString());
...
const gate = run([SELF_RUNTIME, GATE, '--captures', 'console/.impeccable/review/sections']);
if (gate.code !== 0) blocks.push(['look-gate', gate.out.trim().split('\n').filter((l) => /FAIL/.test(l)).join(' · ') || 'blocking check failed']);
