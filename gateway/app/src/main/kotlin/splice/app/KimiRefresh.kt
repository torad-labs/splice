...
// are retryable (3 attempts, backoff 2^attempt seconds, ±10% jitter — see RefreshRetry.kt); the
// X-Msh-* device identity headers ride on the refresh POST too. JsonNull-safe extraction throughout.
package splice.app
...
): KimiRefreshedTokens? = refreshWithRetry(
    call = { postRefresh(client, tokenUrl, refreshToken, identityHeaders) },
    classify = ::classify,
)

private suspend fun classify(resp: HttpResponse): RefreshStep<KimiRefreshedTokens> {
    val status = resp.status.value
    val body = resp.bodyAsText()
    return when {
        resp.status.isSuccess() -> RefreshStep.Terminal(parseKimiRefresh(body))
        isTerminalRefreshFailure(status, body, kimiRefreshJson) -> RefreshStep.Terminal(null)
