#!/usr/bin/env bash
# tools/gate/test/campaign-cli-selftest-canary.sh — the canary for its sibling selftest.
#
...
#
# Method: fixture ROOTs under mktemp -d, each with tools/gate/test/ and a .dev/campaigns/, and a STUB
...
LEG="$ROOT/tools/gate/test/campaign-cli-selftest.sh"
...
# A fixture root: tools/gate/test/<the real leg> + .dev/campaigns/<stub manifest.ts> + the named ledgers.
make_root() {
  local root
  root="$(mktemp -d -t campaign-cli-canary-XXXXXX)"
  fixtures="$fixtures $root"
  mkdir -p "$root/tools/gate/test" "$root/.dev/campaigns"
  cp "$LEG" "$root/tools/gate/test/campaign-cli-selftest.sh"
...
arm() { # arm <name> <expect: RED|GREEN> <root> [diagnosis substring]
  ...
  out="$(bash "$root/tools/gate/test/campaign-cli-selftest.sh" 2>&1)"
  status=$?
