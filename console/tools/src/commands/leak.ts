...
//     file under console/src contained it — and that corpus included the OTHER fixtures, so ten
...
//
// Usage: bun console/tools leak   (from the worktree root; builds first)
import { runLoud } from '../lib/proc.ts';
import { existsSync, readFileSync, readdirSync, statSync } from 'node:fs';
import { join, relative } from 'node:path';

const root = process.cwd();
const consoleDir = join(root, 'console');
const srcRoot = join(consoleDir, 'src');
const distEntry = join(consoleDir, 'dist', 'index.html');
...
// whoever built next.
if (runLoud(['bunx', '--bun', 'vite', 'build'], consoleDir) !== 0) {
  console.error('fixture-leak: the bundle did not build, so there is no dist to search');
  process.exit(1);
}
...
for (const fixture of fixtures) {
  ...
  if (evidence.length === 0) {
    ...
    for (const literal of leaked) {
      console.error(`LEAK ${name}: ${JSON.stringify(literal)} ships in console/dist/index.html`);
    }
  }
}
...
if (missingMarker.length > 0) {
  for (const page of missingMarker) {
    console.error(`MARKER MISSING console/src/pages/${page}: ships a fixture and declares no ${MARKER}, so a capture of it cannot be checked`);
  }
  process.exit(1);
}
if (dist.includes(MARKER)) {
  console.error(`LEAK ${MARKER} ships in console/dist/index.html: it is set inside a DEV-guarded expression, so a production build must drop its name`);
  process.exit(1);
}
