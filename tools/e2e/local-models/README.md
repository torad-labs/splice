...
| streaming | `tools/e2e/probes/stream_probe.ts` validates the Anthropic SSE contract and incremental delivery at the head port |
...
```bash
bun tools/e2e/local-models/e2e.ts --runtime ollama \
  ...
  --out tools/e2e/receipts/local-models/local-models-ollama.json
bun tools/e2e/local-models/e2e.ts --runtime lmstudio \
  --jar app/build/libs/app-all.jar \
  ...
  --out tools/e2e/receipts/local-models/local-models-lmstudio.json
```
