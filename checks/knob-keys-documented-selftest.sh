#!/usr/bin/env bash
# V4-87 — red-green selftest for the knob-key documentation wall.
#
# The wall's proof lives INSIDE the checker, as `--selftest`, because that is the idiom the
# template it mirrors uses (checks/config/quirks-keys-documented.py, gate legs
...
set -euo pipefail
cd "$(dirname "$0")/.."
exec python3 checks/config/knob-keys-documented.py --selftest
