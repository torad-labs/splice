...
# another seat's gradle in the same project dir — the race the slot exists to close. The slot
# supplies --offline (off in CI) and --no-daemon, and wraps buildgate when the box has it.
bun tools/gate slot oss-b -- :daemon-control:test
