...
- **The console sees the turns a head has in flight.** `GET /api/heads` reported every gate's
  ...
  model, or `compact`, and `connect` until the upstream answers, then `streaming`, with its age and
  idle time), its counts since the head started, the mean queue wait, and the head's stream-idle
  limit, all read in one snapshot.
