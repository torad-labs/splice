...
//   console/.impeccable/build/spec.json          region boxes in fractions of the 1536x1024 frame,
//                                              with the measured cap height of every text region
//   console/.impeccable/build/scaffold/layout.css  the same values as `--r-<region>-<key>` customs
...
import { decodePng, hexToRgb } from '../lib/png.ts';
...
import { FIXTURES, urlFor as fixtureUrl } from '../lib/fixtures.ts';
...
import { ROLE_PAIRS, RATIO_LIMIT, checkRatios, compLadder } from '../lib/typography.ts';
...
function comp() {
  const spec = JSON.parse(readFileSync(join(ROOT, 'console/.impeccable/build/spec.json'), 'utf8'));
  const layout = readFileSync(join(ROOT, 'console/.impeccable/build/scaffold/layout.css'), 'utf8');
  const boxes = {};
...
function compPlate(spec, regionId, paperHex, tolerance = 12, majority = 0.5) {
  const png = decodePng(readFileSync(join(ROOT, `console/tools/fixtures/comp/${spec.comp.replace(/^.*mocks\//, '')}`)));
...
const COMP = comp();
const COMP_STRIP_PLATE = compPlate(JSON.parse(readFileSync(join(ROOT, 'console/.impeccable/build/spec.json'), 'utf8')), 'lead-strip', '#DDD8C6');
...
if (wantsHelp) {
  console.log(`usage: bun console/tools comp [--list] [--address <name>] [--json] [--frame WxH]
...
The comp is console/.impeccable/build/spec.json + build/scaffold/layout.css; nothing is retyped.
...
console.log(`  the type ladder below is the ${compLadder(JSON.parse(readFileSync(join(ROOT, 'console/.impeccable/build/spec.json'), 'utf8'))).measured.length} roles it CAN measure, not the whole ladder`);
