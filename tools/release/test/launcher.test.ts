...
describe("the launch shim", () => {
  ...
  // UF-04 and UF-05, V4-258's boot-log flag, and the CLI console arm (v0.4.0 review).
  test("the rehearsal is the script's arms, all of them", () => {
    expect(ARM_NAMES.length).toBe(6 + 2 + SELECTORS.length + 2 + 1 + 1);
    expect(ARM_NAMES.length).toBe(21);
    expect(ARM_NAMES.filter((name) => name.startsWith("UF-03")).length).toBe(SELECTORS.length);
    for (const marker of ["JW-01", "JW-04", "UF-01", "UF-02", "UF-04", "UF-05", "V4-258"]) {
