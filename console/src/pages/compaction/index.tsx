...
export function RulesSection({ instructions, error = null, lastRead = null }: {
  ...
  const columns: Column<InstructionRule>[] = [
    { key: 'scope', label: S.scope, width: '12%', cell: (rule) => rule.scope },
    { key: 'source', label: S.source, width: '34%', mono: true, primary: true, cell: (rule) => rule.source },
    { key: 'length', label: S.length, width: '24%', cell: (rule) => lengthCell(rule, longest) },
    {
      key: 'heads',
      label: S.heads,
      cell: (rule) => <span className="myx-cp-heads">{rule.heads.map((head) => <HeadMark key={head} head={head} />)}</span>,
    },
  ];
  return (
    <Section title={S.rules} {...(instructions === null ? {} : { count: rules.length })}>
      ...
      {instructions === null ? null : rules.length === 0 ? (
        ...
        <DataTable columns={columns} rows={rules} rowKey={(rule) => `${rule.scope}:${rule.source}`} label={S.rules} />
      )}
    </Section>
  );
}
