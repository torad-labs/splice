...
//
// THE COMP OF RECORD is console/tools/fixtures/comp/team-board-a.png and its measured numbers come from
...
//
//   bun console/tools typography [--address teams] [--json]
//   bun console/tools typography --selftest
import { readFileSync, writeFileSync, mkdirSync } from 'node:fs';
import { join, resolve } from 'node:path';
import { pathToFileURL } from 'node:url';
import { mgmtKey, show, withChrome } from './cdp.ts';
import { urlFor } from './fixtures.ts';
import { colorFraction, decodePng, hexToRgb } from './png.ts';

const ROOT = resolve(import.meta.dirname, '../../../..');
const OUT = join(ROOT, 'console/.impeccable/review/type-ladder');
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log(text);
  console.log(`\nwritten to console/.impeccable/review/type-ladder/ladder.txt`);
