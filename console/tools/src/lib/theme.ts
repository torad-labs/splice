...
//
// Usage: bun console/tools theme --selftest
//        bun console/tools theme '<url>' [--theme both|light|dark] [--out DIR] [--width W --height H]
import { mkdirSync } from 'node:fs';
import { readFileSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import process from 'node:process';
import { mgmtKey, shoot, show, withChrome } from './cdp.ts';
...
export function seedingSites(files) {
  return [...files.entries()]
    .filter(([path, text]) => !path.endsWith('theme.ts') && text.includes(`'${SEED_KEY}'`))
    .map(([path]) => path)
    .sort();
}
...
async function selftest() {
  ...
  const synth = new Map([
    ['lib/theme.ts', `the key is '${SEED_KEY}' and this file is allowed to say it`],
  ...
  try {
    ...
    const r = run([process.execPath, probe]);
    check('importing this file returns instead of running its CLI',
      r.code === 0 && r.out.includes('RETURNED function'),
      `exit ${r.code}, out ${JSON.stringify(r.out.trim().slice(0, 60))}`);
    const withFlag = run([process.execPath, probe, '--selftest']);
    check('and still returns when the IMPORTER was run with --selftest',
      withFlag.code === 0 && withFlag.out.includes('RETURNED function'),
      `exit ${withFlag.code}`);
  } finally {
  ...
  const walk = (dir, prefix = '') => readdirSync(dir).sort().flatMap((name) => {
    ...
    return statSync(full).isDirectory() ? walk(full, rel) : (name.endsWith('.ts') ? [[rel, readFileSync(full, 'utf8')]] : []);
  });
  const tree = new Map(walk(resolve(HERE, '..')));
  const live = seedingVerdict(tree);
  check(`ONE implementation in the whole toolbox (${tree.size} .ts walked)`,
    tree.size > 0 && live.unexempt.length === 0,
    live.unexempt.length === 0 ? 'no file but theme.ts names the key' : `SECOND IMPLEMENTATION: ${live.unexempt.join(', ')}`);
...
if (isMain && (url === undefined || ARGS.includes('--help'))) {
  console.log("usage: bun console/tools theme '<url>' [--theme both|light|dark] [--out DIR] [--width W --height H]");
  console.log('       bun console/tools theme --selftest');
