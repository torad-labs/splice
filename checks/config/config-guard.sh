#!/usr/bin/env bash
# checks/config/config-guard.sh — rules that guard the RULES (#924 Phase 0.5, eli C2). Once inline
...
# this check entirely. Structure is the denominator now: checks/rules/ast-grep-rule-docs.py parses
...
python3 checks/rules/ast-grep-rule-docs.py severity . || fail=1
...
# other here, and `"gate:concentration": "python3 checks/rules/concentration.py --top 5"` is a one-line
# edit that exits 0 forever while checks/gate.sh keeps printing a green concentration leg. Same
# completeness shape checks/rules/rule-routing.sh applies to ast-grep rule directories, one surface up.
