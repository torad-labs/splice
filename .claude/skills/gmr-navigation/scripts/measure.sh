...
# --- Core block files (1-7) ---
echo "## Core Blocks (1-7)"
BLOCK_PASS=0
BLOCK_FAIL=0
for block in \
  "blocks/1-anchor.md" \
  "blocks/2-vertex-scan.md" \
  "blocks/3-multi-pass.md" \
  "blocks/4-theta-graph.md" \
  "blocks/5-phi-inversion.md" \
  "blocks/6-filter-137.md" \
  "blocks/7-verification.md"; do
...
# --- Block 8: Document ---
echo "## Block 8: Document (blocks/8-document/)"
B8_PASS=0
B8_FAIL=0

if [ -f "$SKILL_DIR/blocks/8-document/README.md" ]; then
  echo "  FOUND: blocks/8-document/README.md"
  B8_PASS=$((B8_PASS + 1))
else
  echo "  MISSING: blocks/8-document/README.md"
  B8_FAIL=$((B8_FAIL + 1))
fi

for sub in \
  "blocks/8-document/8a-gather.md" \
  "blocks/8-document/8b-synthesis.md" \
  "blocks/8-document/8c-audit.md" \
  "blocks/8-document/8d-structure.md" \
  "blocks/8-document/8e-attention.md" \
  "blocks/8-document/8f-design.md" \
  "blocks/8-document/8g-build.md" \
  "blocks/8-document/emphasis-organism-plan.md" \
  "blocks/8-document/emphasis-rejection-spec.md"; do
  if [ -f "$SKILL_DIR/$sub" ]; then
    LINES=$(wc -l < "$SKILL_DIR/$sub" | tr -d ' ')
    echo "  FOUND: $sub ($LINES lines)"
    B8_PASS=$((B8_PASS + 1))
  else
    echo "  MISSING: $sub"
    B8_FAIL=$((B8_FAIL + 1))
...
echo "  Result: $B8_PASS/10 found, $B8_FAIL missing"
echo ""

# --- Block 9: Carousel ---
echo "## Block 9: Carousel (blocks/9-carousel/)"
B9_PASS=0
B9_FAIL=0

if [ -f "$SKILL_DIR/blocks/9-carousel/README.md" ]; then
  echo "  FOUND: blocks/9-carousel/README.md"
  B9_PASS=$((B9_PASS + 1))
else
  echo "  MISSING: blocks/9-carousel/README.md"
  B9_FAIL=$((B9_FAIL + 1))
fi

for sub in \
  "blocks/9-carousel/9a-extract.md" \
  "blocks/9-carousel/9b-align.md" \
  "blocks/9-carousel/9c-content.md" \
  "blocks/9-carousel/9c2-skeleton.md" \
  "blocks/9-carousel/9d-voice.md" \
  "blocks/9-carousel/9e-antibody.md" \
  "blocks/9-carousel/9f-fascia.md" \
  "blocks/9-carousel/9g-layout.md" \
  "blocks/9-carousel/9h-animation.md" \
  "blocks/9-carousel/9i-assembly.md"; do
  if [ -f "$SKILL_DIR/$sub" ]; then
    LINES=$(wc -l < "$SKILL_DIR/$sub" | tr -d ' ')
    echo "  FOUND: $sub ($LINES lines)"
    B9_PASS=$((B9_PASS + 1))
...
echo "  Result: $B9_PASS/11 found, $B9_FAIL missing"
echo ""
...
REF_FAIL=0
for ref in \
  "references/primes-and-geometry.md" \
  "references/worked-example.md" \
  "references/pipeline-overview.json"; do
...
ORPHAN_COUNT=0
for subdir in reference references scripts examples assets blocks phases evals; do
