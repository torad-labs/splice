...
class DoctorLocalRuntimeTest {
    ...
    fun `a runtime whose model list does not answer is one WARN row, not a FAIL per model`() {
        ...
        val checks = DoctorLocalRuntime(DoctorTestPorts.local(listless))
    ...
    fun `only local providers are checked, rows answer for themselves, live rows appear only on request`() {
        ...
        val checks = DoctorLocalRuntime(DoctorTestPorts.local(up)).localChecks(topology, live = false)
        ...
        val live = DoctorLocalRuntime(DoctorTestPorts.local(up)).localChecks(topology, live = true)
    ...
    fun `the live probe loads the model and the verdict reads the served window it reveals`() {
        ...
        val topology = TopologyLoader.parse(toml)
        val cold = DoctorLocalRuntime(DoctorTestPorts.local(loading)).localChecks(topology, live = false)
        assertEquals(CheckStatus.OK, cold.first { it.name == "local:ollama/qwen3:4b" }.status, "unloaded: card only")
        val hot = DoctorLocalRuntime(DoctorTestPorts.local(loading)).localChecks(topology, live = true)
    ...
    fun `doctor applies the daemon's per-head window override in both directions`() {
        ...
        val wide = DoctorLocalRuntime(DoctorTestPorts.local(up), override = { _, _ -> 65536L })
        ...
        val narrow = DoctorLocalRuntime(DoctorTestPorts.local(up), override = { _, _ -> 8192L })
    ...
    fun `the real config override reads the head's own overrides table like the daemon does`(@TempDir tmp: Path) {
        ...
        val wide = DoctorLocalRuntime(DoctorTestPorts.local(up), env, ConfigHeadWindowOverride(env))
        ...
        val narrow = DoctorLocalRuntime(DoctorTestPorts.local(up), env, ConfigHeadWindowOverride(env))
        assertEquals(CheckStatus.OK, narrow.localChecks(TopologyLoader.parse(narrowed), live = false).row().status)
        val plain = DoctorLocalRuntime(DoctorTestPorts.local(up), env, ConfigHeadWindowOverride(env))
        assertEquals(CheckStatus.FAIL, plain.localChecks(TopologyLoader.parse(declared), live = false).row().status)
    }

    private fun List<DoctorCheck>.row() = first { it.name == "local:ollama/qwen3:4b" }

    @Test
    fun `the head's effective rows are checked, not the provider table`() {
        val overriding = toml.replace("port = 3901", "port = 3901\ncontext_window = 65536")
        val checks = DoctorLocalRuntime(DoctorTestPorts.local(up))
            .localChecks(TopologyLoader.parse(overriding), live = false)
        ...
        val stripped = DoctorLocalRuntime(DoctorTestPorts.local(up))
            .localChecks(TopologyLoader.parse(suffixed), live = false)
