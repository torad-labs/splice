...
// FILE SCOPE ON PURPOSE: one configured Json parser shared by every claim decode. As a member it
// would be rebuilt per CodexOAuth construction, and decodeJwtClaims runs on the auth-read path.
private val jwtJson = Json { ignoreUnknownKeys = true }

/** The codex OAuth wire builders, JWT claim reader and auth.json shaper. Stateless — collaborators
 *  construct one. */
