...
function page(over: Partial<TranscriptPage> = {}): TranscriptPage {
  return { session_id: 's1', path: '/home/marcos/.claude/projects/x/s1.jsonl', messages: [], ...over };
}
...
describe('groupSessions', () => {
  ...
  test('files by repo, falling back to the cwd until the resolver lands', () => {
    const groups = groupSessions(
      [
        session({ repo: { root: '/dev/mythos' } }),
        session({ repo: { root: '/dev/mythos', worktree: '/dev/mythos/.claude/worktrees/v0.4.0' } }),
