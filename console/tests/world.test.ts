...
//   - the OLD TOKENS are `git show main:console/src/shared/tokens.css` minus the names section 1
...
function sectionOneNames(): Set<string> {
  ...
  const contract = readFileSync(path.join(repoRoot, '.dev/campaigns/web-console/CONTRACTS.md'), 'utf8');
...
/** The sheet's path on the base branch: its current home first, then where it lived before the
 *  console restructure (chore/restructure-4), so the wall reads a base that predates the move. */
const BASE_SHEETS = ['console/src/shared/tokens.css', 'webui/src/shared/tokens.css'] as const;

function baseTokenSheet(): string {
  for (const ref of BASE_REFS) for (const sheet of BASE_SHEETS) {
    try {
      return execSync(`git show ${ref}:${sheet}`, {
        cwd: repoRoot,
        encoding: 'utf8',
  ...
  throw new Error(
    `the world wall cannot read its denominator: none of ${BASE_REFS.join(', ')} resolves ` +
      `${BASE_SHEETS.join(' or ')}. On a runner, fetch the base branch before the console tests ` +
