...
//     before this row: :254 returns 'console/tools/src holds no .ts files, so the denominator is
...
//
// Usage: bun console/tools coverage
//        bun console/tools coverage --selftest
...
const DIR = join(ROOT, 'console/tools/src');
const EXIT_GATE = 'commands/exit.ts';
// The verb -> file table the CLI dispatches with, so a leg that names a verb names a file here.
...
export function legsOf(text) {
  ...
  // a leg names a file two ways: a verb it hands the CLI (`tool('leak'`), resolved through the
  // dispatcher's own table, or a legacy path under .dev/web-console it still runs with node
  for (const m of block.matchAll(/name:\s*'([^']+)'|\btool\('([a-z-]+)'|dev\/web-console\/([A-Za-z0-9._/-]+\.mjs)(?![A-Za-z0-9._/-])/g)) {
    if (m[1] !== undefined) { current = m[1]; if (!byLeg.has(current)) byLeg.set(current, []); }
    // VERBS is relative to console/tools; this denominator is relative to console/tools/src
    else if (current !== null && m[2] !== undefined) byLeg.get(current).push(VERBS[m[2]]?.replace(/^src\//, '') ?? `verb:${m[2]}`);
    else if (current !== null) byLeg.get(current).push(m[3]);
  }
...
export function guard({ files, legs, ledger = null, needsLedger = false }) {
  if (files === null) return 'no console/tools/src directory — this is not the worktree root';
  if (files.length === 0) return 'console/tools/src holds no .ts files, so the denominator is empty and nothing could be checked';
  if (!files.includes(EXIT_GATE)) return `console/tools/src holds no ${EXIT_GATE} — there is no gate to have coverage of`;
...
export function audit({ files, dispositions, legs, sources, ledger = '' }) {
  ...
  for (const file of files) {
    ...
    if (d === undefined) {
      rows.push({ file, kind: null, verdict: UNDISPOSITIONED,
        detail: 'in console/tools/src and in none of the four dispositions — absence is not one of them' });
      continue;
    }
    ...
    if (d.kind === LEG) {
      ...
      // console/.impeccable/review/coverage/coverage.mjs — a row's own review script, not a toolbox
      ...
      if (!sources.has(caller)) {
        ...
        const outside = files.includes(caller) ? '' : ' (outside console/tools/src)';
  ...
  const stale = Object.keys(dispositions).filter((f) => !files.includes(f))
    .map((file) => ({ file, kind: dispositions[file].kind, verdict: STALE,
      detail: 'dispositioned here and not in console/tools/src any more' }));
...
async function selftest() {
  ...
  const legs = new Map([['alpha', [EXIT_GATE, 'a.mjs']], ['beta', ['b.mjs']]]);
  const sources = new Map([['a.mjs', "import x from './lib.mjs'"], ['t.mjs', 'no imports here']]);
  const base = {
    [EXIT_GATE]: { kind: GATE, why: 'the gate' },
  ...
  const files = ['a.mjs', 'b.mjs', EXIT_GATE, 'lib.mjs', 't.mjs'];
  ...
  ok('a tree whose every checker is correctly disposed is all OK', run(base),
    { 'a.mjs': OK, 'b.mjs': OK, [EXIT_GATE]: OK, 'lib.mjs': OK, 't.mjs': OK });
  ...
  ok('a tree with no commands/exit.ts is DID NOT RUN', guard({ files: ['a.mjs'], legs }) !== null, true);
  ok('an exit-gate.mjs whose LEGS array will not parse is DID NOT RUN',
    guard({ files: [EXIT_GATE], legs: null }) !== null, true);
  ...
  ok('a verb handed to the CLI names the file the dispatcher maps it to',
    [...(legsOf("const LEGS = [\n{ name: 'x', run: () => tool('leak') },\n];\n") ?? new Map()).values()].flat(), ['commands/leak.ts']);
  ok('and the scan leg is found to run commands/scan.ts', (realLegs?.get('scan') ?? []).includes('commands/scan.ts'), true);
  ...
  const probe = join(DIR, 'lib', '.gate-coverage-selftest-probe.ts');
  const { writeFileSync, rmSync } = await import('node:fs');
  const { run: runProc } = await import('./proc.ts');
  const self = fileURLToPath(import.meta.url);
  const live = () => {
    const r = runProc([process.execPath, self], ROOT);
  ...
  try {
    ...
    const planted = live();
    ok('a NEW .ts dropped into the directory with no disposition turns it RED', planted.status !== 0, true);
    ok('and the red names the file rather than only counting it',
      planted.out.includes('.gate-coverage-selftest-probe.ts'), true);
  } finally {
...
if (process.argv.includes('--selftest')) {
  ...
  // a LIBRARY OF claim may name a caller outside console/tools/src; read it from the worktree root so
  ...
  console.log(`gate-coverage: ${files.length} checker(s) under console/tools/src, ${legs.size} leg(s) in ${EXIT_GATE}`);
  ...
  if (bad.length > 0) {
    ...
    console.error('REMEDY: give each one a disposition in lib/coverage.ts — GATE LEG, LIBRARY OF <file>, ' +
      'or TOOL, NEVER GATES with the reason — or wire it into commands/exit.ts if it should have been a leg all along.');
  } else {
